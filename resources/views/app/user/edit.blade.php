                    ...
                    <div class="col-md-12 grid-center stretch-card">
                        ...
                        <a href="{{route('user.index')}}"
                                ...
                                <form method="POST" action="{{route('user.update', $user->id)}}" class="forms-sample"
                                    ...
                                    @csrf
                                    @method('patch')
                                    <h4 class="card-title text-white mt-4 mb-4"disabled>{{$user->firstName}} {{$user->lastName}}</h4>
                                    
                                    <div class="form-group">
                                        <label for="email" class="form-label">E-mail</label>
                                        <input type="email" class="form-control text-white" id="email" name="email" autofocus  value="{{ $user->email ?? old('email')}}" 
                                        @if ($errors->has('email'))
